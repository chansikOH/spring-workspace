...
public class CustomExcelView extends AbstractExcelView {
	...
	protected void buildExcelDocument(Map<String, Object> model, HSSFWorkbook workbook, HttpServletRequest request,
		...
		Integer buy = (Integer) model.get("buy");
		Integer sell = (Integer) model.get("sell");
		Integer profit = (Integer) model.get("profit");
		
		// 새로운 엑셀 시트를 생성한다.
		HSSFSheet sheet = workbook.createSheet("sample sheet");
		// 엑셀 시트에 새로운 행을 생성한다.
		HSSFRow row = sheet.createRow(0);
		// 생성된 행의 첫 새로운 칸을 생성하고, 값을 설정한다.
		row.createCell(0).setCellValue("title");
		
		HSSFRow row1 = sheet.createRow(1);
		row1.createCell(0).setCellValue("총매입 금액");
		row1.createCell(1).setCellValue(buy);
		
		HSSFRow row2 = sheet.createRow(2);
		row2.createCell(0).setCellValue("총매출 금액");
